...
def _get_files(subject, data_dir):
    ...
    img_files = sorted(Path(
        data_dir, 'things.fmriprep', 'sub-01'
    ...
    events = sorted(Path(
        data_dir, 'things.fmriprep', 'sourcedata', 'things', 'sub-01'
...
def _gen_xmatrix_contrast(img, event, mask, t_r=1.49, smoothing_fwhm=5):
    """
    Parameters
    ----------
    img : str or Pathlike
    ...
    return design_matrix, stats_img
...
if __name__ == "__main__":
    ...
    for img, event, mask in zip(img_files, events, masks):
        design_matrix, stats_img = _gen_xmatrix_contrast(img, event, mask)
